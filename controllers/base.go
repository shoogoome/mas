...
func (this *ControllerBase) Verification() {
	...
	if config.SystemConfig.Server.Token != token{
...
func (this *ControllerBase) RedisConn() redis.Conn {
	...
	connectString := fmt.Sprintf(
		"%s:%d",
			config.SystemConfig.Redis.Host,
		config.SystemConfig.Redis.Port,
		)
	conn, err := redis.Dial("tcp", connectString)
	if err != nil {
		this.Exception(http_err.RedisConnectExcept())
	}
	if _, err := conn.Do("AUTH", config.SystemConfig.Redis.Password); err != nil {
		_ = conn.Close()
