...
func (this *FileSystemController) saveFile(ddbyte []byte, fileInfo models.FileInfo, hash string) {
	...
	for index, shard := range shards {
		fileInfo.StorageServerIp = append(fileInfo.StorageServerIp, <-ips)
		client := <-clients
		go service.GRPCUpload(client, shard, index, fileInfo, statusMap)
	}
		...
		if !status.Status {
			...
			if resend[status.Index] < config.SystemConfig.Server.Resend {
				go service.GRPCUpload(status.Client,
					shards[status.Index],
					status.Index, fileInfo, statusMap)
	...
	except = dao.SaveFileInfo(fileInfo)
	if except != nil {
		this.Exception(except)
	}
}
